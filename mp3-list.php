...
<li><a href="http://<?php echo $sitehost ?>/index.php">已上传文件</a></li>
<li><a href="http://<?php echo $sitehost ?>/bcs.html">上传文件[已开放]</a></li>
<li class="active"><a href="http://<?php echo $sitehost; ?>/mp3-list.php">MP3 List</a></li>
...
$sql = "SELECT count(id) FROM baefile_info WHERE ext = \"mp3\"";
$result=mysql_query($sql)OR die('<p>读取数据库错误！</p>');
$row=mysql_fetch_array($result);
$file_num=$row[0];
//计算已经占用了多少空间
$sql="SELECT sum(msize) FROM baefile_info WHERE ext = \"mp3\"";
$result=mysql_query($sql)OR die('<p>读取数据库错误！</p>');
$row=mysql_fetch_array($result);
$file_size=$row[0];
...
$count=mysql_query("SELECT COUNT(*) FROM baefile_info WHERE ext = \"mp3\"");
$rs=mysql_fetch_array($count); 
$totalNumber=$rs[0];
$totalPage=ceil($totalNumber/$perNumber);
$page=isset($_GET['page'])?intval($_GET['page']):1; //如果没有值,则赋值1
$startCount=($page-1)*$perNumber;
$result=mysql_query("SELECT * FROM baefile_info WHERE ext = \"mp3\" ORDER BY datetime DESC LIMIT $startCount,$perNumber");
echo '<p>以下是本站MP3文件列表共有'.$totalPage.'页。本站目前有 '.$file_num.' 个MP3文件，MP3文件大约使用了 '.$file_size.' MB，'; include 'capacity.php'; echo '</p>';
...
if ($page != 1) {echo '<a href="http://'.$sitehost.'/mp3-page-1" ><strong>首页</strong></a><a href="http://'.$sitehost.'/mp3-page-'.($page - 1).'" class="pagelist"><strong>上一页</strong></a>';}
for ($i=1;$i<=$totalPage;$i++) if($page < $limit_page && $i < $limit_page){echo '<a href="http://'.$sitehost.'/mp3-page-'.$i.'" class="pagelist">'.$i.'</a>';}
if ($page<$totalPage) {echo '<a href="http://'.$sitehost.'/mp3-page-'.($page + 1).'" class="pagelist">下一页</a><a href="http://'.$sitehost.'/mp3-page-'.$totalPage.'" class="pagelist">尾页</a>';}
...
for ($i=1;$i<=$totalPage;$i++) echo '<option value="http://'.$sitehost.'/mp3-page-'.$i.'">'.$i.'</option>';
